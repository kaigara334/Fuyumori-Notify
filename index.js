...
client.once(Events.ClientReady, async () => {
  ...
  const commands = [
    new SlashCommandBuilder()
      .setName('operationnotify')
      .setDescription('冬森鉄道運行情報を通知します')
      .addStringOption(option =>
        option.setName('date')
          .setDescription('日付を入力してください（例: 2024-10-31）')
          .setRequired(true))
      .addStringOption(option =>
        option.setName('starttime')
          .setDescription('運行開始時間を入力してください（例: 10:00 JST）')
          .setRequired(true))
      .addStringOption(option =>
        option.setName('endtime')
          .setDescription('運行終了時間を入力してください（例: 14:00 JST）')
          .setRequired(true))
      .addStringOption(option =>
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  if (interaction.commandName === 'operationnotify') {
    // 必要なロールがあるかを確認
    ...
    if (!hasRole) {
      // ロールがない場合エラーメッセージを表示
      return await interaction.reply({ content: `このコマンドを実行するには、\`${roleName}\` ロールが必要です。`, ephemeral: true });
    }
    ...
    await interaction.reply({ content: "運行情報が送信されました！", ephemeral: true });
  }
});
